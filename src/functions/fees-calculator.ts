...
export default function (orderItems: FixedOrderItems, orders: Order[]): [string, { [key: string]: number }] {
    ...
    orders.forEach(order => {
        ...
        order.order_items.forEach((orderItem) => {
            ...
            const fee = orderItems[orderItem.type].fees.reduce((acc: number, fee: Fee) => {
            ...
            orderFee += fee;
            finalOutput += `   Order item ${chalk.italic(orderItem.type)}: ${currencyFormater(fee)}\n`;
        });

        totals[order.order_number] = orderFee;

        finalOutput += '\n   ' + chalk.underline('Order total: ' + (currencyFormater(orderFee))) + '\n\n';
    });
